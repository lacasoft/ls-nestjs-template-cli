...
- [Sistema de Configuración del Sistema](#-sistema-de-configuración-del-sistema)
...
## ⚙️ Sistema de Configuración del Sistema

El proyecto incluye un sistema de **configuración dinámica** que permite ajustar parámetros del sistema en tiempo de ejecución sin necesidad de reiniciar la aplicación.

### Configuraciones Disponibles

Las configuraciones se almacenan en la base de datos (tabla `system_config`) y se pueden modificar a través de la API admin:

**Configuraciones Generales:**

- `defaultCurrency` - Moneda por defecto del sistema (ej: USD, MXN, EUR)
- `trialDaysDefault` - Días de prueba gratuita para nuevas cuentas
- `maintenanceMode` - Activar/desactivar modo mantenimiento
- `maxFailedLoginAttempts` - Intentos máximos de login antes de bloqueo
...
- `invoiceNumberPrefix` - Prefijo para números de factura

**Configuraciones de Seguridad:**

- `passwordMinLength` - Longitud mínima de contraseñas
- `sessionTimeoutMinutes` - Tiempo de expiración de sesión

**Configuraciones de Negocio:**

- `maxLocationsPerAccount` - Máximo de ubicaciones por cuenta
- `maxUsersPerAccount` - Máximo de usuarios por cuenta
- `paymentReminderDaysBefore` - Días antes del pago para enviar recordatorio
- `subscriptionGracePeriodDays` - Días de gracia después de vencimiento

**Características (Features):**

- `emailNotifications` - Habilitar notificaciones por email
- `smsNotifications` - Habilitar notificaciones SMS/push
...
### Seeder de Configuración

El sistema incluye valores por defecto que se crean automáticamente:

```bash
# Ejecutar seeder (incluye configuración del sistema)
npm run seed:admin
```

Ver [ADMIN_API_EXAMPLES.md](ADMIN_API_EXAMPLES.md#configuración-del-sistema) para más ejemplos.
